    ...
    <template #content>
      <UCard>
        ...
        <div class="space-y-6 py-2">
          <div>
            <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Appearance</h4>
            <div class="space-y-4">
              ...
              <div class="flex items-center justify-between">
                 <div class="flex flex-col">
                  ...
                  <span class="text-xs text-gray-500 dark:text-gray-400">Disables grain & cursor effects</span>
                </div>
                <USwitch v-model="reducedMotion" color="primary" />
              </div>

              <UFormField label="Theme Color">
                <div class="flex gap-2 overflow-x-auto py-2 px-4 -mx-4">
                  <button
                    ...
                    :class="[
                      primaryColor === color.name ? 'ring-gray-400 dark:ring-gray-500 scale-110' : 'ring-transparent'
                    ]"
            ...
            >
              ...
              <UTooltip v-if="!secretEnabled" :text="lockTooltip">
                <UIcon 
                  ...
                  :class="clickCount > 0 ? 'text-primary-400' : 'text-gray-400'"
                />
              </UTooltip>
            ...
            <div class="space-y-4 pt-2">
              ...
              <div class="flex items-center justify-between">
                <div class="flex flex-col">
                  ...
                  <span class="text-xs text-gray-500 dark:text-gray-400">Adds a trail based on brainrot level</span>
                </div>
                <USwitch v-model="cursorTrailEnabled" color="error" :disabled="reducedMotion" />
              </div>
            </div>
          </template>
        </div>
...
const grayColor = computed({
  ...
  set: (val) => { appConfig.ui.colors.neutral = val }
})
...
const handleSecretClick = () => {
  ...
  if (clickCount.value === 2) {
     toast.add({ title: 'Hmm?', description: 'The lock seems loose...', duration: 1500, color: 'gray', icon: 'i-heroicons-lock-closed' })
  } else if (clickCount.value >= 5) {
     secretEnabled.value = true
     clickCount.value = 0
     toast.add({ title: 'Unlocked!', description: 'You smashed the lock!', icon: 'i-heroicons-lock-open', color: 'green' })
  }
}
